...
try {
  ...
  if (process.env.VAPID_PUBLIC && process.env.VAPID_PRIVATE) {
    webpush.setVapidDetails(
      'mailto:admin@rps.com',
      process.env.VAPID_PUBLIC,
      process.env.VAPID_PRIVATE
    );
    console.log("Push notifications enabled.");
  } else {
    console.log("Push notifications disabled (no VAPID keys set).");
  }
} catch(e) {
...
app.post("/auth/request-otp", async (req, res) => {
  ...
  try {
    ...
    // Log FIRST so it always shows even if email fails
    console.log(`\n=== OTP GENERATED ===\nEmail: ${email}\nCode: ${code}\n====================\n`);

    // Check if SMTP is configured
    if (!process.env.SMTP_USER || !process.env.SMTP_PASS) {
      console.log("SMTP not configured — OTP only available in logs above.");
      // Still return ok:true so user can proceed to OTP step
...
// POST /admin/add-user  { username, forceRelease? }
app.post("/admin/add-user", requireAdminHeader, async (req, res) => {
  const username = (req.body.username || "").trim().slice(0, 20);
  const forceRelease = req.body.forceRelease || false;
  if (!username) return res.json({ ok: false, error: "Username required." });
  try {
    // Check registered
    const exists = await db.getUserByUsername(username);
    if (exists) return res.json({ ok: false, error: `@${username} is already a registered account.` });

    // Check active guest
    const activeGuest = await db.getActiveGuestByUsername(username);
    if (activeGuest && !forceRelease) {
      const days = Math.floor((Date.now()/1000 - activeGuest.updated) / 86400);
      return res.json({
        ok: false,
        guestConflict: true,
        error: `@${username} is currently used by a guest (active ${days} day${days!==1?'s':''} ago). Force release or wait until they're inactive for 5 days.`,
      });
    }

    // Release guest if forced
    if (activeGuest && forceRelease) {
      await db.releaseGuestUsername(username);
    }
...
// GET /audio/clips — list all clips (no data)
app.get("/audio/clips", async (req, res) => {
  try { res.json(await db.getAudioClips()); }
  catch (e) { res.json([]); }
});

// GET /audio/clip/:id — get base64 data for a single clip
app.get("/audio/clip/:id", async (req, res) => {
  try {
    const clip = await db.getAudioClipData(req.params.id);
    if (!clip) return res.status(404).json({ error: "Not found." });
    // Return as audio file
    const buf = Buffer.from(clip.data, 'base64');
    res.set('Content-Type', clip.mimetype || 'audio/mpeg');
    res.set('Cache-Control', 'public, max-age=86400');
    res.send(buf);
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// POST /admin/audio/upload — upload audio as base64 to DB
app.post("/admin/audio/upload", requireAdminHeader, async (req, res) => {
  const name     = decodeURIComponent(req.headers['x-clip-name']     || 'Clip');
  const emoji    = decodeURIComponent(req.headers['x-clip-emoji']    || '🔊');
  const mimetype = req.headers['x-clip-mimetype'] || 'audio/mpeg';
  const type     = req.headers['x-clip-type']     || 'chat'; // chat, win, lose, draw

  const allowed = ['audio/mpeg','audio/wav','audio/ogg','audio/mp4','audio/webm','audio/x-m4a'];
  ...
  try {
    ...
    req.on('end', async () => {
      ...
      if (buf.length > 5 * 1024 * 1024) {
        return res.json({ ok: false, error: 'File too large. Max 5MB.' });
      }
      const base64 = buf.toString('base64');
      const clip = await db.addAudioClip(name, emoji, base64, mimetype, type);
      io.emit('clips_updated', await db.getAudioClips());
...
// POST /admin/audio/delete  { id }
app.post("/admin/audio/delete", requireAdminHeader, async (req, res) => {
  try {
    await db.deleteAudioClip(req.body.id);
    io.emit('clips_updated', await db.getAudioClips());
    res.json({ ok: true });
  } catch (e) { res.json({ ok: false, error: e.message }); }
});
...
app.get("/profile/:username", async (req, res) => {
  ...
  try {
    ...
    let user = await db.getProfile(username);
    if (user) {
      ...
      return res.json({
        username: user.username, type: 'registered',
        bio: user.bio||'', avatar_emoji: user.avatar_emoji||'🎮',
        avatar_color: user.avatar_color||'#7c6aff',
        wins: user.wins||0, losses: user.losses||0, draws: user.draws||0,
        win_pct: total ? Math.round((user.wins||0)/total*100) : 0,
        win_streak: user.win_streak||0, best_streak: user.best_streak||0,
        created: user.created,
        badges: badges.map(b=>{ const def=db.BADGE_DEFS.find(d=>d.key===b.badge_key); return {...def,earned_at:b.earned_at}; }).filter(Boolean),
        history,
      });
    }

    // Try guest
    const guest = await db.getGuestByUsername(username);
    if (guest) {
      const history = await db.getMatchHistory(username, 20);
      const total   = (guest.wins||0)+(guest.losses||0)+(guest.draws||0);
      return res.json({
        username: guest.username, type: 'guest',
        bio: guest.bio||'', avatar_emoji: guest.avatar_emoji||'🎮',
        avatar_color: guest.avatar_color||'#7c6aff',
        wins: guest.wins||0, losses: guest.losses||0, draws: guest.draws||0,
        win_pct: total ? Math.round((guest.wins||0)/total*100) : 0,
        win_streak: guest.win_streak||0, best_streak: guest.best_streak||0,
        created: guest.updated,
        badges: [], history,
      });
    }
...
// POST /profile/update  { username, bio, avatar_emoji, avatar_color, ip? }
app.post("/profile/update", async (req, res) => {
  const { username, bio, avatar_emoji, avatar_color, ip } = req.body;
  ...
  try {
    ...
    // Guest update by IP
    if (ip) {
      await db.updateGuestProfile(ip, { bio, avatar_emoji, avatar_color });
      return res.json({ ok: true });
    }
...
app.post("/friends/search", async (req, res) => {
  ...
  try {
    ...
    console.log(`Online-only results:`, onlineGuests.map(r=>r.username));

    const all = [...reg.rows, ...guests.rows, ...onlineGuests]
      .filter((v,i,a) => a.findIndex(x=>x.username===v.username)===i)
      .slice(0,10)
      .map(u => ({ username: u.username, type: u.type, status: onlineUsers.get(u.username)?.status || 'offline' }));
...
async function recordResult(player, result, opponent, myScore, oppScore, goal) {
  try {
    if (player.type === "registered") {
      ...
      const newBadges = await db.checkAndAwardBadges(player.username, 'registered');
      ...
      // Guest — update stats in guests table
      ...
      // Save match history using username as key
      await db.addMatchHistory(player.username, opponent, result, myScore, oppScore, goal);
      // Check badges for guest
      const newBadges = await db.checkAndAwardBadges(player.username, 'guest', player.ip);
      if (newBadges.length > 0) {
        const s = onlineUsers.get(player.username);
        if (s) {
          const defs = newBadges.map(k=>db.BADGE_DEFS.find(d=>d.key===k)).filter(Boolean);
...
io.on("connection", socket => {
  ...
  socket.on("identify", async ({ type, email, username }) => {
    try {
      if (type === "admin") {
        // Admin is a real player account too
        const user = await db.getUserByUsername(ADMIN_USERNAME);
        socket.identity = { type:"registered", username: ADMIN_USERNAME, email: user?.email||null, ip: getIP(socket), isAdmin: true };
        onlineUsers.set(ADMIN_USERNAME, { socketId: socket.id, status: 'online' });
        const lb = await db.getLeaderboard();
        socket.emit("identified", { username: ADMIN_USERNAME, leaderboard: lb, isAdmin: true });
        return;
      }
      if (type === "registered") {
        const user = email
        ...
        socket.identity = { type:"registered", email: user.email||null, username: user.username, ip: getIP(socket), isAdmin };
      } else {
      ...
      socket.emit("identified", {
        ...
        leaderboard: lb,
        isAdmin: socket.identity.isAdmin || false,
        ip: socket.identity.type === 'guest' ? socket.identity.ip : undefined,
      });
    } catch (e) {
      console.error("identify error:", e.message, e.stack);
      socket.emit("auth_error", "Server error: " + e.message);
    }
  });

  socket.on("join_queue", () => {
    if (!socket.identity || socket.identity.isAdmin) return;
  ...
  // ── Audio clip in match ──
  socket.on("send_audio_clip", clipId => {
    if(!socket.roomId || !socket.identity) return;
    socket.to(socket.roomId).emit("audio_clip_received", {
      clipId,
      from: socket.identity.username,
    });
  });

  // ── Audio clip in chat (direct to friend) ──
  socket.on("send_audio_dm", ({ to, clipId }) => {
    if(!socket.identity) return;
    const toSocket = onlineUsers.get(to);
    if(toSocket) {
      io.to(toSocket.socketId).emit("audio_clip_received", {
        clipId,
        from: socket.identity.username,
        isDM: true,
      });
...
// ══════════════════════════════════════════════════════════════════ START
Promise.all([db.init(), db.initFriends()]).then(async ()=>{
  // Expire inactive guests (5+ days)
  await db.expireInactiveGuests().catch(e=>console.error("expiry error:", e.message));

  // Auto-create admin account if it doesn't exist
  try {
    const adminExists = await db.getUserByUsername(ADMIN_USERNAME);
    if (!adminExists) {
      await db.createUser(null, ADMIN_USERNAME, true);
      console.log(`Admin account "${ADMIN_USERNAME}" created.`);
    }
  } catch(e) { console.error("Admin account setup error:", e.message); }
