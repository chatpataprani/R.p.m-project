...
async function sendPushNotification(username, title, body, data = {}) {
  if (!webpush || !process.env.VAPID_PUBLIC || !process.env.VAPID_PRIVATE) return;
  try {
...
app.post("/auth/request-otp", async (req, res) => {
  ...
  try {
    ...
    if (!process.env.SMTP_USER || !process.env.SMTP_PASS) {
      ...
      return res.json({ ok: false, error: "Email not configured on server. Check Render logs for your code." });
    }
    ...
    res.json({ ok: false, error: `Email failed: ${e.message}. Check Render logs for your code.` });
  }
});
...
app.post("/friends/search", async (req, res) => {
  ...
  try {
    ...
    const reg = await pool.query(
      ...
      [`%${query}%`, me || '']);
    // Search active guests (updated within 5 days)
    const fiveDaysAgo = Math.floor(Date.now()/1000) - (5*24*60*60);
    const guests = await pool.query(
      `SELECT username, 'guest' AS type FROM guests WHERE username ILIKE $1 AND username != $2 AND updated >= $3 LIMIT 5`,
      [`%${query}%`, me || '', fiveDaysAgo]);
    // Merge, deduplicate, sort
    const all = [...reg.rows, ...guests.rows]
